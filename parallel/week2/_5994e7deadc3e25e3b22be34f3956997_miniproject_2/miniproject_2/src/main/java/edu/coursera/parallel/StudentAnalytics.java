...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        	double retval = Stream.of(studentArray).parallel().filter(s -> s.checkIsCurrent()).mapToDouble(a -> a.getAge()).average().getAsDouble();
        	return retval;
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        	Map<String, Long> retVal = Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent()).collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting()));
        	
        	String mostCommonName = null;
        	long mostCommon = -1;
        	
        	for(String key : retVal.keySet()) {
        		if(retVal.get(key) > mostCommon) {
        			mostCommonName = key;
        			mostCommon = retVal.get(key);
        		}
        	}
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
    		int retVal = (int)Stream.of(studentArray).parallel().filter(s -> s.getGrade() < 65 && s.getAge() > 20 && !s.checkIsCurrent()).count();
    		return retVal;
    }
}
